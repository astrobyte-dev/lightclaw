...
function DevelopActions.applySettings(params)
    ...
    -- Also push values through LrDevelopController so the active Develop panel
    -- updates in real time.  applyDevelopSettings writes to the catalog but the
    -- live develop controller holds a separate in-memory copy that may not
    -- refresh until the user switches photos.
    if updated > 0 then
        pcall(function()
            for key, value in pairs(params.settings) do
