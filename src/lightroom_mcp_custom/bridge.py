...
class BridgeConnectionError(BridgeError):
    """Raised when the bridge cannot connect to the Lightroom plugin sockets."""
...
class BridgeConfig:
    host: str = "127.0.0.1"
    port_file: Path = field(
        default_factory=lambda: Path(tempfile.gettempdir()) / "lightroom_mcp_custom_ports.json"
    )
...
class AsyncLightroomBridge:
    """Two-connection bridge to the Lightroom Lua plugin.

    Architecture:
    - rx connection (to send_port): Python reads responses/events from here.
      Lua's mode="send" socket sends responses to connected Python clients.
    - tx connection (to receive_port): Python writes commands here.
      Lua's mode="receive" socket fires onMessage, dispatches, then sends
      the response via the senderSocket (rx connection).
    """
        ...
        async with self._lock:
            ...
            while time.monotonic() < deadline:
                ...
                try:
                    ...
                    # Plugin's "receive" socket -> we write commands to it.
    ...
    async def _send_command_once(
        ...
        # Write commands to the receive_port (tx_writer).
        ...
        if not response.get("success", False):
            ...
            msg = err.get("message", "unknown Lightroom error")
            raise BridgeCommandError(f"{command} failed ({code}): {msg}")
        ...
        try:
            while True:
                ...
                if not raw:
                    continue
                if raw.lower() == "ok":
                    # LrSocket connection acknowledgement, not a JSON payload.
                    continue
